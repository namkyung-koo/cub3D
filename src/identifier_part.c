...
/*   Updated: 2023/06/28 18:24:55 by nakoo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_identifier(t_data *data, char *line)
{
	...
	if (data->flag.is_north_texture == 1 || data->flag.is_south_texture == 1 \
	|| data->flag.is_east_texture == 1 || data->flag.is_west_texture == 1)
	{
		if (save_texture_path(data, temp) == -1)
			exit (1);
	}
	if ((data->flag.is_floor == 1 && data->floor_rgb == -2) \
	|| (data->flag.is_ceiling == 1 && data->ceiling_rgb == -2))
	{
